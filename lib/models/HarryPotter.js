...
module.exports = class HarryPotter {
  ...
  static async updateById(id, details) {
    const existingCharacter = await HarryPotter.findById(id);
    const updatedDetails = { ...existingCharacter, ...details };
    const { name, type, patronus } = updatedDetails;
    const { rows } = await pool.query(
      `
      UPDATE
      harrypotter
